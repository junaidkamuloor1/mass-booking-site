...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const grid1 = document.getElementById("movie-grid");

    grid1.innerHTML = Object.entries(movies).map(([slug, movie]) => `
        <article class="movie-card" onclick="window.location.href='pages/showtimes.html?movie=${slug}'" style="cursor: pointer;">
            <div class="poster">
    ...
    const grid2 = document.getElementById("c-movie-grid");

    grid2.innerHTML = Object.entries(comingSoon).map(([slug, movie]) => `
    <article class="movie-card" onclick="window.location.href='pages/showtimes.html?movie=${slug}'" style="cursor: pointer;">
        <div class="poster">
            <img src="${movie.poster}" alt="${movie.title}">
        </div>
        ...
        <div class="movie-info">
            <h3>${movie.title}</h3>
            <span>Opens on ${movie.release}</span>
        </div>
    </article>
    `).join("");
});
...
function openBooking(theatre, movie, time) {
    ...
    bookingContainer.classList.remove('confirmed', 'payment'); // updated line
...
const PRICE_PER_SEAT = 249.99; // demo pricing only

document.querySelector('.step-seats .book-btn').onclick = () => {
...
function goToPayment() {
    ...
    document.getElementById('paySummaryMovie').textContent =
        `${currentBooking.movieTitle} · ${selectedSeats.length} seat${selectedSeats.length > 1 ? 's' : ''}`;
    ...
    bookingContainer.classList.add('payment');
}

function autofillCard() {
    document.getElementById('cardNumber').value = '4242 4242 4242 4242';
    document.getElementById('cardExpiry').value = '12/29';
    document.getElementById('cardCvv').value = '123';
    document.getElementById('cardName').value = 'A. MOVIEGOER';
}

const autofillBtn = document.getElementById('autofillBtn');
if (autofillBtn) {
    autofillBtn.addEventListener('click', autofillCard);
}
    document.getElementById('payBtn').onclick = () => {
        // Demo only — no real validation or processing happens here.
        const ticket = {
        ...
        bookingContainer.classList.remove('payment');
